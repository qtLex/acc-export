...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не РежимОтладки Тогда
		...
		Сообщить("Не удалось штатно завершить работу. Выходим аварийно!");
...
Процедура ЗапускПроверки()
	...
	Лог_Отладка("Начало проверки конфигурации " + ТекущаяДата());
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Лог_Отладка("При выполнении проверки произошла ошибка: ");
		Лог_Отладка(ТекстОшибки);
		
	КонецЕсли;
	
	Лог_Отладка("Окончание проверки конфигурации " + ТекущаяДата());
...
Процедура УстановитьЗаголовокФормы()
	...
	Лог_Отладка("Версия обработки " + ВерсияОбработки);
...
Процедура ПрочитатьФайлПараметров(Знач ПутьКФайлуПараметров)
	...
	Если Не фФайлСуществует(ПутьКФайлуПараметров) Тогда
		Лог_Отладка(СтрШаблон("Файл параметров %1 не найден.", ПутьКФайлуПараметров));
		Возврат;
	КонецЕсли;
...
Процедура ОбеспечитьАбсолютныйПутьККаталогуИсходныхКодов()
	...
	Лог_Отладка("Не удалось определить тип выгрузки. Возможно каталог исходных кодов задан не верно.");
...
Функция НайтиКаталогИсходныхКодов()
	...
	Если ЭтоПолныйПуть
		...
		Лог_Отладка("Каталог исходных файлов по переданному полному пути: " + КаталогИсходныхКодов);
	...
	Если Не ЭтоПолныйПуть Тогда
		...
		Иначе
			...
			Лог_Отладка("Не удалось определить каталог исходных кодов как [каталог проекта] + [каталог исходных кодов]");
...
Процедура ПостОбработкаВывестиПараметрыВСообщения()
	...
	Лог_Отладка("Имя проекта = " + ИмяПроекта);
	Лог_Отладка("Конфигурация = " + Конфигурация + ", код: " + Конфигурация.Код);
	Лог_Отладка("Каталог проекта = " + КаталогПроекта);
	Лог_Отладка("Файл результата = " + ИмяФайлаРезультата);
	Лог_Отладка("Каталог исходников = " + КаталогИсходныхКодов);
	Лог_Отладка("Выполнять проверку = " + ВыполнятьПроверку);
	Лог_Отладка("Формат экспорта = " + ФорматЭкспорта);
	Лог_Отладка("Выгружать правила = " + ВыгружатьПравила);
